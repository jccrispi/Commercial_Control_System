...
if ($_SERVER['REQUEST_METHOD'] == 'GET') 
{
	...
	if(!$qtdeprod && !$qtdeserv){
		echo  "<h2><center>NO PRODUCTS OR SERVICES REGISTERED FOR THIS SALE!</center></h2>";
		echo "<a href='consult_venda.php' target='principal'><h2>New Search</h2></a>";
	}
	else
	{

		echo '<h2><center>SALES DETAILS</center></h2>';
		...
		if($qtdeserv){			
			...
			$sql .= " venda.codigovend = $codvenda and recai.codvenda = $codvenda GROUP BY codigoserv";
					...
					echo '<tr>';
					echo '<td bgcolor="#E0DFEE"><h2><center>TOTAL SALES VALUE</h2></td>';
					$totalvenda = $totprod + $totserv;
					echo '<td bgcolor="#E0DFEE"><h2>$ ' . number_format($totalvenda,2, ',', '.').'</h2></td>';
					echo '</tr>';
					echo '</table>';
